...
private:
  ...
  winrt::Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice device_winrt{
      nullptr};
...
void ScreenCapturerImpl::StartCapture() {
  ...
  try {
    ...
    frame_arrived_revoker = frame_pool.FrameArrived(
        winrt::auto_revoke, {this, &ScreenCapturerImpl::OnFrameArrived});
...
void ScreenCapturerImpl::OnFrameArrived(
  ...
  // Check for resize
  if ((contentSize.Width != last_size.Width) ||
      (contentSize.Height != last_size.Height)) {
    last_size = contentSize;
    frame_pool.Recreate(device_winrt, DirectXPixelFormat::R16G16B16A16Float, 2,
                        last_size);
