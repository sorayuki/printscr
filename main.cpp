...
int main() {
  ...
  try {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      if (frame) {
        std::cout << "Frame captured! " << frame->metadata.width << "x"
                  << frame->metadata.height << std::endl;
        break;
      }
    }
    ...
    if (selection.IsValid()) {
      std::cout << "Selection confirmed: (" << selection.Left() << ", "
                << selection.Top() << ") to (" << selection.Right() << ", "
                << selection.Bottom() << ")" << std::endl;
      std::cout << "Size: " << selection.Width() << "x" << selection.Height()
                << std::endl;
    } else {
